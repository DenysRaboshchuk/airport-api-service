...
class Route(models.Model):
    source = models.ForeignKey(
        Airport, on_delete=models.CASCADE, related_name="outgoing_routes"
    )
    destination = models.ForeignKey(
        Airport, on_delete=models.CASCADE, related_name="incoming_routes"
    )
...
class Ticket(models.Model):
    ...
    def clean(self):
        ...
        if self.row > airplane.rows:
            errors["row"] = (
                f"Row {self.row} is invalid. Airplane has only {airplane.rows} rows."
            )
        if self.seat > airplane.seats_in_row:
            errors["seat"] = (
                f"Seat {self.seat} is invalid. Airplane has only {airplane.seats_in_row} seats."
            )
